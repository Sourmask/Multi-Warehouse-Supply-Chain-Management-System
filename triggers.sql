...
BEGIN
    ...
    -- Step 2: Find the nearest warehouse (least difference)
    OPEN wh_cursor;
    read_loop: LOOP
    ...
    -- Step 3: If warehouse found, assign order and update status
    IF v_closest_warehouse IS NOT NULL THEN
        INSERT INTO warehouse_queue (order_id, warehouse_id, assigned_time, notes)
        VALUES (NEW.order_id, v_closest_warehouse, NOW(), 'Auto-assigned by trigger');
